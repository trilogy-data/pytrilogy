...
DuckDB database loaded with the ACME Insurance benchmark. The DDL below is the
schema the tables were created from (`ACME_small.ddl`, as published with the
benchmark); write standard DuckDB SQL against these tables.

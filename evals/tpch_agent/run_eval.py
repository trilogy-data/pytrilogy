#!/usr/bin/env python
"""On-demand eval: drive the Trilogy agent CLI through a TPC-H modeling task
...
run manually. See evals/README.md.
    ...
    python evals/tpch_agent/run_eval.py [--model ...] [--scale-factor ...]
"""
...
def prepare_worker_workspace(src: Path, worker_idx: int) -> Path:
    ...
    shutil.copy2(src / "tpch.duckdb", worker_dir / "tpch.duckdb")
...
def install_enriched_model(workspace: Path, src_dir: Path) -> dict:
    ...
    memory.<table>``), but the eval workspace's ``tpch.duckdb`` attaches as a
    ...
    copied: list[str] = []
    rewrites = 0
    # cache*.preql in the tpc_h test dir holds persist/dashboard helpers
    # that aren't part of the model the agent should see — skip alongside
    # the answer queries.
    for path in sorted(src_dir.glob("*.preql")):
        name = path.name
        if (
            name.startswith("query")
            or name.startswith("adhoc")
...
def render_markdown(report: dict) -> str:
    ...
    out: list[str] = []
    out.append(f"# TPC-H Agent Eval — {meta['timestamp']}")
    out.append("")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--scale-factor", type=float, default=0.01, help="TPC-H scale factor"
    )
    parser.add_argument(
...
def main() -> int:
    ...
    print(f"[1/5] Building TPC-H DuckDB (sf={args.scale_factor:g}) ...")
    cached = db.build_database(args.scale_factor, EVAL_DIR / ".cache")
    workspace_db = db.copy_database(cached, workspace / "tpch.duckdb")
    ...
    # paying engine setup + `INSTALL tpch; LOAD tpch;` on every grade.
    try:

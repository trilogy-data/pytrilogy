...
each generated query against the TPC-H reference (``PRAGMA tpch(n)``)."""
...
def make_scoring_engine(db_path: Path, workspace: Path):
    """Build a Trilogy executor on the workspace's duckdb, with TPC-H loaded.
...
def score_queries(db_path: Path, workspace: Path, ids: list[int]) -> list[QueryResult]:
    ...
    the TPC-H reference for that query id (``query<id>.preql`` vs
    ``PRAGMA tpch(<id>)``)."""
...
def _score_one(engine, workspace: Path, idx: int) -> QueryResult:
    ...
    try:
        reference = list(engine.execute_raw_sql(f"PRAGMA tpch({idx});").fetchall())
    except Exception as exc:
        return QueryResult(
            id=idx,
            status="error",

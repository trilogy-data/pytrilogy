"""Build and cache a DuckDB database loaded with TPC-DS data.
...
def cache_path(cache_dir: Path, scale_factor: float) -> Path:
    return cache_dir / f"tpcds_sf{scale_factor:g}.duckdb"


def build_database(scale_factor: float, cache_dir: Path) -> Path:
    """Return a cached DuckDB file populated with TPC-DS at ``scale_factor``,
    generating it via the duckdb ``tpcds`` extension on first use."""
    import duckdb
    ...
    try:
        ...
        # dsdgen is a lazy table function whose row generation is the side
        # effect -- it must be materialized for the tables to be created.
        con.execute(f"SELECT * FROM dsdgen(sf={scale_factor})").fetchall()

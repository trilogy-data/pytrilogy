...
`describe_unresolvable_filter(output_concepts)` (`trilogy/core/processing/discovery_utility.py`) runs when discovery returns no node **after** the disconnected-subgraph check, at both generic emission points (`source_query_concepts` in `concept_strategies_v3.py` and the v4 dead-end in `query_processor.py`). It's a generic FILTER de-opacifier: for any `Derivation.FILTER` output left in the unbuildable set, it renders the real filter expression (`r.wk ? <condition>`) and names the condition's real concepts instead of the hashed `_virt_filter_<hash>` address. When the filtered value is a rowset output it additionally lists the rowset's columns and which condition concept isn't among them:

> Could not build filter `r.wk ? <condition>`: its condition references `sales.date.year`, which could not be sourced together with the value being filtered (`r.wk`). Rowset `r` exposes only {`r.amt`, `r.dow`, `r.wk`}, and `sales.date.year` is not among them. A rowset is materialized at its own grain, so reference one of its outputs, add the condition concept(s) to rowset `r`'s select, or filter the underlying base concept before/inside the rowset instead.

Design notes (from review feedback that the first cut was "overly specific to rowsets and to this filter condition"):
- **Generic, not rowset-only.** Fires for any unresolved FILTER concept; the rowset-output detail is added context, not the trigger.
- **No over-claim.** It does NOT assert "out of grain / aggregated away". Empirically, pseudonyms are empty at this stage, so filtering by the BASE name of a concept the rowset DID expose under an alias (`sales.date.day_of_week` exposed as `r.dow`) also lands here — but the fix there is "reference `r.dow`", not "add to grain". Hence the remedies are phrased as options, the first being "reference one of its outputs".
- **Not aggregate.** A FILTER over a plain `sum(x) by k` aggregate by an out-of-grain concept is *satisfiable* (it just re-grains) and resolves fine — only materialized/islanded rowsets fail this way. So there is no aggregate branch.
- **Pure message refiner.** Only runs when root is None; no behavior change. In-grain (`? r.dow=0`) and base-concept filters still resolve.

**Manifestation 2 is unchanged**: it splits into >1 subgraph, so the earlier disconnected-subgraph check raises its named-subgraph `DisconnectedConceptsException` first and this never runs. Tests: `test_non_benchmark_queries.py::test_q02_filter_rowset_output_by_out_of_grain_concept_clean_error` (inline + named_auto), `::test_q02_filter_rowset_output_by_alias_base_name_clean_error` (over-claim guard), `::..._by_in_grain_concept_still_resolves`.

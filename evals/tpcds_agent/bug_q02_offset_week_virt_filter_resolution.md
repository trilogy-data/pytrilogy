...
### FIXED 2026-06-28 (Manifestation 1 — this IS a disconnected grouping; route it through the standard error)

Insight (from review): filtering a rowset output by an aggregated-away concept is *the same problem* as any disconnected grouping — `r.wk` (a materialized/islanded rowset output) and `sales.date.year` live in different reference-graph components, and the fix is "add a join or merge" (or add the concept to the rowset). The only reason it dead-ended on the opaque `_virt_filter_<hash>` was that the condition concept (`sales.date.year`) is hidden inside the FILTER concept's lineage, so the top-level disconnect check (outputs + WHERE row-args) never saw it. Verified: feeding the FILTER's `content` + `where.row_arguments` into `disconnected_components` splits the set into `{r.wk}` and `{sales.date.year}`.

Fix = `raise_if_filter_disconnected(output_concepts, environment, g, extra_required=...)` in `discovery_utility.py`, called in the `root is None` path *after* the existing disconnect check at both `source_query_concepts` (`concept_strategies_v3.py`) and the v4 dead-end (`query_processor.py`). It re-runs `disconnected_components` with FILTER outputs' hidden concepts surfaced (`_filter_hidden_concepts`); if that splits, it raises the **standard** `DisconnectedConceptsException` / `format_disconnected_subgraphs_error` message, plus a rowset-specific suffix (`filter_disconnect_context`):

> Discovery error: cannot merge all concepts into one connected query. The requested concepts split into 2 disconnected subgraphs: {sales.date.year}; {r.wk}. Are you missing a join or merge statement to relate them? Here `sales.date.year` is referenced only inside a filter on rowset output `r.wk` (rowset `r`), and isn't related to it without a join. Relate them by adding `sales.date.year` to rowset `r`'s select, by an existence comparison against a base-concept set (e.g. `r.wk in (<base concept> ? <condition>)`), or by joining the rowset back to the source.

Design notes:
- **General message, specific context.** Same `DisconnectedConceptsException` and "add a join or merge" line as every other disconnected grouping; the rowset filter remedy is an appended suffix, not a bespoke message. (Earlier iterations tried a fully bespoke `UnresolvableQueryException` — rejected as over-specific and over-claiming.)
- **No aggregate branch.** A FILTER over a plain `sum(x) by k` aggregate by an out-of-grain concept is *satisfiable* (it re-grains) and resolves fine — only materialized/islanded rowsets disconnect this way.
- **Additive / no behavior change otherwise.** Only runs when discovery already failed (root is None) and only *after* the existing disconnect check, so non-filter disconnects and Manifestation 2 are untouched. The alias-by-base-name case (`? sales.date.day_of_week`, exposed as `r.dow`) routes through the same path honestly — the message names the two disconnected concepts and the suffix's "filter the base concept / add to rowset" still applies.

**Manifestation 2 is unchanged**: its outputs already split (`{cur.wk, nxt.amt}; {_virt_filter_amt}`) at the earlier check, which raises first. Tests: `test_non_benchmark_queries.py::test_q02_filter_rowset_output_by_out_of_grain_concept_clean_error` (inline + named_auto, asserts `DisconnectedConceptsException` + "join or merge" + rowset hint), `::..._by_in_grain_concept_still_resolves`.

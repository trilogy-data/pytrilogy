...
- `X` is a **value, not a KEY**. A key in a grouping grain is a FINAL merge axis rather than a carried column, so hosting one re-shapes how contributors stitch. That is a second, separate reason from coverage: `order_id` is completely bound and covered, and hosting it still made a `sum(qty)` at `item_id` two-keyed (see the cross-pairing section below; its fix clears 11 of the 13 failures key hosting caused, and the rule stays for the other two). Values (a fact property, a dimension attribute, a scalar over either) ride without touching the join topology, and are row-correct inside a two-`~` span;
...
**Still open, a second route to the same pairing.** When a span is also a grouping key of another aggregate (`min(qty) by user_id` beside `sum(qty)` on the composite model), `user_id` is never a peel candidate: it stays on the fact bucket, which pads the user family there, while `~product_id` peels and pads its own. No clusters to merge, and the BASIC merge over the two reunites them with the same null-safe join. Folding the peel back into the fact bucket fixes that model and breaks `test_forked_with_status` (the product extension row is lost), so it is not the fix. The pairing itself belongs to merge join typing: two sides padded by *different* spans must never pair their padding, which is a rule for `join_resolution`'s padding provenance, not for bucketing. Pinned as the strict xfail `test_composite_grain_families_with_by_span_aggregate`; under key hosting it is also what `test_forked_with_status` and `test_forked_full_column_set` hit.

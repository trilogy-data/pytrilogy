...
Found on the way, recorded, not changed: `select undelivered_customer` alone now returns `('ann',), (None,)` on the derived model, where the plan's owner answer 5 expects the filtered set `('ann',)`; `select customer_id, undelivered_customer` already returns the plan's expected `(1,'ann'), (2,NULL), (3,NULL)` on the derived model as a side effect of phase 4, with no test pinning either. Both belong to owner answer 5's change. `_keep_extension_families_together` is NOT dead: raising inside its merge branch fails 12 tests across the processing, optimization and engine suites.

### Pick up here

0. **Phases 4 and 5 are built; read their sections above before touching them.** The oracle owes one query and one test is a strict xfail beside it, both named in 2. Every `zquery<N>.log` is byte-identical to before phase 4; phase 5's own A/B is recorded in its section.
1. **Owner answer 5 is owed as its own change, outside the keyspace**: a `filter` concept in a SELECT must be a value, NULL where the predicate fails, never a restriction of the other columns' rows. The seam is `v4_node_generators/filter.py::gen_filter`: it pushes the concept's own predicate into the node's WHERE whenever every non-filter output is a grain key of the filter's CONTENT (`pushable_siblings`), and again for an aggregate predicate when `siblings_constant`. Both carve-outs are documented there as intended ("a key like `product_id` beside `name ? product_id%2=0` is safe") and both are what the owner called a bug: the push is only right when the filter concepts are the node's ONLY outputs. The value rendering already exists (the row-preserving `CASE WHEN`, collapsed to the content's grain by `CTE.filter_collapses_to_grain`). Expected rows are drafted in the 2026-09-21 session notes: `select customer_id, late_name` keeps every customer, `select customer_id, undelivered_customer` is `(1,'ann'), (2,NULL), (3,NULL)`, `select late_name` alone stays the filtered set. Not started: it reaches every model, so measure the blast radius (tests pinning the narrowed rows) before editing expectations, and report it rather than rewriting them wholesale.
...
4. Phase 3's two parked heal improvements can be tried now (merged `~` keys, `status = 'delivered'`-style pins): drop `null_rejected=` from `partial_bridging._statement_keyspace` and A/B with rows. Derived null-rejections are true in the rendered SQL only where a domain was built.

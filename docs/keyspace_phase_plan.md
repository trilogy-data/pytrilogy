...
- **A direct non-null proof names the column as the CTE renders it** (`optimizations/join_upgrade._downgrade`): it forces the side in the CTE's `source_map`, never the other side's copy of the key. The FINAL INNER-joins an aggregate over the region on `s.sk`; that equality flowed back as an external proof on the aggregate's input merge, where `s.sk` renders from the domain (the preserved side), and was read as proving the solid side's key, turning the domain's LEFT into INNER (`EQUIVALENT_SPELLINGS`, the multi-aggregate keyed rowset). A COALESCE over both sides proves neither.

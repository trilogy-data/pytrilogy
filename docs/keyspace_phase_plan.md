...
3. **Phase 5 is built** (its section above). What it leaves: the audit's `where` emitter is still wired (`audit_undelivered_where`) and fires for an atom whose inputs are neither carried nor absent-and-rows-only, a shape no suite or oracle query has; and `_preserved_final_branch` still stands beside the keyspace answer.

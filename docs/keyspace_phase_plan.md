...
> **Where this stands (2026-09-21): phases 0-4 are BUILT, on branch `extension-row-null-semantics`, draft PR #702. Phase 5 (WHERE by region) is next, and owner answer 5 (a filter is a value) is owed as its own change. A fresh session should read "The problem" and "The idea" for the vocabulary, then jump to [Status](#status) and start from "Pick up here".** The sections before Status are the original plan, left unedited on purpose: where they say "nothing here is built" or list a site as re-deriving its own answer, Status is the truth.
...
0. **Phase 4 is built; read its section above before touching it.** The oracle owes one query and one test is a strict xfail beside it, both named in 2. Every `zquery<N>.log` is byte-identical to before phase 4: no TPC-DS or TPC-H benchmark statement has a demanded region with an absent null-opaque derivation.
...
3. **Phase 5 (WHERE by region) has a worklist**: the audit's `where` rows (`TRILOGY_KEYSPACE_AUDIT`), one per region that needed a domain and got none because no host delivers an atom to the domain's rows (`_filters_region_domain`: not carried by the domain, and not an absent value read from rows alone). Those statements keep the padded plan, which is the one mixed-semantics boundary left. Known member: `where activity = 'dormant'`, a scalar over an aggregate (it also still fails to render, `Missing source map entry for local.order_id`, pre-existing).
4. Phase 3's two parked heal improvements can be tried now (merged `~` keys, `status = 'delivered'`-style pins): drop `null_rejected=` from `partial_bridging._statement_keyspace` and A/B with rows. Derived null-rejections are true in the rendered SQL only where a domain was built, so expect the `where` audit rows to be the exceptions.

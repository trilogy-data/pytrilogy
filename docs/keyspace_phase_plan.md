...
> **Where this stands (2026-09-24): phases 0-5, owner answer 5 (a filter is a value) and a review cleanup pass are BUILT, on branch `extension-row-null-semantics`, draft PR #702. Phases 6-7 remain and continue on the same PR. A fresh session should read "The problem" and "The idea" for the vocabulary, then jump to [Status](#status) and start from "Pick up here".** The sections before Status are the original plan, left unedited on purpose: where they say "nothing here is built" or list a site as re-deriving its own answer, Status is the truth.
...
0. **Phases 4 and 5 and owner answer 5 are built; read their sections above before touching them.** The oracle owes two queries and one test is a strict xfail beside them, named in 2. Every `zquery<N>.log` but q05 and q41 (answer 5's section) is byte-identical to before phase 4; the branch's final A/B is recorded at the end of phase 5.
1. **Owner answer 5 is built** (its section above). Left: `select late_name, count(order_id)` in `OWED`; a HAVING aggregate riding the mandatory list blocks the sole-output push (q41, equivalent rows).
2. Phase 4 leftovers, each a strict xfail so a fix shows up as XPASS:
   - `select name as n2, count(status) as n` returns `('cat', NULL)`, owed `('cat', 0)`. The count's zero-fill (`dialect/base.py`, a nullable COUNT read through a join) is skipped when the reading CTE groups, which the FINAL does once a renamed group key moved to the domain.
   - `test_unsold_item_counts_no_lines[ROWSET_QUERY]`: a rowset body is its own plan with its own keyspace, and pads inside it (`('gamma', 1, 0)`). Same root as "a rowset as a witness" below.
   - A row-stream BUCKET that mixes present and absent derivations (`item_desc as d` beside the line hash) is not split the way a ROOT bucket is; the rename rule above covers the rename, nothing covers a present non-rename (`upper(name)` beside `status` in one BASIC group). No test yet.
3. **Phase 5 is built** (its section above). What it leaves: the audit module is deleted; its `where` emitter is now a planner log line in `region_domains.add_region_domain_buckets` ("keeps its padded plan"), firing for an atom whose inputs are neither carried nor absent-and-rows-only, a shape no suite or oracle query has; and `_preserved_final_branch` still stands beside the keyspace answer.
4. Pin-heal on derived null-rejections is DONE (answer 5's section). The other parked phase 3 heal improvement, merged `~` keys, is still untried.
5. Not modelled: a rowset as a witness, related to model keys only through an authored scoped join (`tree_in_play_spans` is the stopgap for join typing; 2's rowset xfail is the same gap for placement). The OPTIONAL entity is modelled after all, see phase 4.
6. Phases 6 and 7 inherit more than the plan listed: `_add_region_domain_contributors` (moves ALIAS renames onto the domain and hides columns on built nodes at FINAL assembly; the group graph should decide it), the hidden-output rule in `MergeNode`'s shortcut, the sibling-grain widening and `kept_axis` in `_assemble_final_node` (all "rows versus columns"; `StrategyNode.region_spans` is the contract to extend), the `span_scope` plumbing, and two ownership mechanisms in `elect_extent_owners` (elected owners permit ancestors, domain owners permit descendants minus `_solid_groups`; see the sizing experiment above). `_keep_extension_families_together` is likely dead once every demanded family has a domain; not checked.
7. Latent, found on the way and fixed: two merges of the same members typed differently shared a CTE (`_null_extended_sides`). Latent and NOT fixed: the solid stream still joins the dimension it has no use for (`customers INNER JOIN orders` to read `orders.customer_id`), because the root bucket's re-source prefers a complete binder; it is correct and wasteful.

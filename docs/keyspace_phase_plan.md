...
> **Where this stands (2026-09-24): phases 0-5 and owner answer 5 (a filter is a value) are BUILT, on branch `extension-row-null-semantics`, draft PR #702. Phases 6-7 remain. A fresh session should read "The problem" and "The idea" for the vocabulary, then jump to [Status](#status) and start from "Pick up here".** The sections before Status are the original plan, left unedited on purpose: where they say "nothing here is built" or list a site as re-deriving its own answer, Status is the truth.
...
6. Phases 6 and 7 inherit more than the plan listed: the `keep=` parameters on `_fold_passthrough_parents` / `_drop_ancestor_parents`, `_add_region_domain_contributors`, the hidden-output rule in `MergeNode`'s shortcut (all "rows versus columns"), and the span plumbing `BuildEnvironment.in_play_spans` / `extent_free_carried` beside `extent_free_spans`. `_keep_extension_families_together` is likely dead once every demanded family has a domain; not checked.

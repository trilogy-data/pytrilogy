...
- **A scalar aggregate `by *` over a `~` region is rows-correct as planned** (TPC-H q22, once listed here as "keeps its padded plan"): `avg(bal) by *` under `then where ... and order_id is null` is evaluated over its argument's key rows, each customer once whatever stage 1 rejected on the order side, and the LEFT JOIN + `is null` plan is the anti-join. Eight spellings against reference SQL in `tests/engine/test_scalar_aggregate_over_region.py`. The emitter fires on q22 and on three of those spellings in a full-suite sweep, and not in a tpc_h-only run (the order dependence under Process): the padded plan is the verified one.
...
- **The FULL narrowing (`grain_utility.downgrade_join_for_proofs`) forces a side by a side-only column only when that column is bound completely and plainly there.** A value the domain carries is PARTIAL on the solid stream (`_extent_free_partials`), so `where item_name != 'B'` over a `~?` model (the merge is FULL: the guest's key is a value NULL) no longer keeps the solid side and drops the unsold item. The key-tuple rule keeps forcing through a `~` key: it is what keeps an undemanded region's rows out (`select order_id, status where customer_id in (1, 3)`, the oracle). Plan-shape cost, open: TPC-H instantiated `adhoc07` (SQL-only) now keeps two FULLs at FINAL (`lineitem-side FULL order_sales FULL customers`, coalesced keys) that its WHERE over a solid-side-only column null-rejects the padding of, rows identical; the optimizer's `UpgradeJoinOnGuards` should narrow them and does not.
- **A collapsed CTE keeps its partial marks** (`optimizations/utils.carry_child_state`). The optimizer's `_blocked_partials` is what stops a non-null proof on a key from forcing a side that binds it PARTIALLY; the solid pre-merge of an aggregate over the region carries the span partial (`_extent_free_partials`), and `CollapseSingleParent` folded that projection into its GROUP and dropped the mark. The FINAL INNER-joins the aggregate on `s.sk`; that equality flowed back as an external proof on the aggregate's input merge, where `s.sk` renders from the domain, and with nothing blocked it was read as proving the solid side's key, turning the domain's LEFT into INNER (`EQUIVALENT_SPELLINGS`, the multi-aggregate keyed rowset). Scoping the proof to the rendering side instead was tried: it also stopped a proof on a complete dimension's key (TPC-DS q64/q23 dim lookups went LEFT).
...
- **A key no handle spells is spelled by the handle keyed on it alone** (`s.d` for `item_sk`): that handle is the region's present handle, its span, and its padding's name. With several such handles the one that cannot be NULL comes first (a `string?` stand-in pairs its NULL member with a NULL-keyed guest and vetoes the join into a FULL), then by name. Every other handle keyed on that key LIVES ON the stand-in (`RowsetWitness.entity_handles` are the reader's declared keys in `build_keyspace`): reading `s.n` alone still resolves to the `s.d` region. Before that, the reader saw one region and counted the unsold item's padded hash (`test_stand_in_beside_a_second_property`). If there is no such handle, the region stays out of the reader's plan: probed with a body demanding the region only through an aggregate argument (`count(item_sk) by order_number`), and the reader is right to read the rowset as a row source (the NULL-`o` row is a real row, and a CASE over `s.n_items` evaluates on it, where the direct spelling's CASE over an aggregate by `order_number` is absent there).
...
1. **A guest's NULL through a `~?` key on a non-`?` property is a VALUE, and is read as absence** (strict xfails in `test_stand_in_key_pairs_the_guest`). `select s.n, count(grain(s.o, s.n))` over `rowset s <- select order_number as o, item_name as n, ...` on the `~?` guest model loses the guest's NULL group: the rowset's domain (the boundary, which holds the guest's row with `n` NULL) LEFT-joins the aggregate on a plain `=`. Two causes, traced: (a) the solid boundary strips `item_name`'s nullability as extension padding (`extension_padded_addresses` counts any join keyed on the span, but a LEFT onto the dimension pads only the `?` rows: extension rows pad the FACT's columns, on the side that binds the span partially), so no side is nullable on `s.n`; (b) once it is nullable, `extent_null_addresses` classifies the chained `?` NULL as extent and `_unpaired_value_nulls` vetoes on the stand-in (it is `held`), where a PROPERTY's NULL through a NULL key is a value that must pair null-safely (a KEY's stays a veto: a guest names no member). The direct spelling is right only because its merge is FULL with a plain `=`; with a NULL-named item in the dimension it would return two NULL rows. Probe: `local_scripts/keyspace_ab/probes/probe_witness_leftovers.py`.
2. **Direct-path bugs on the `~?` guest model, pre-existing on main** (found by the same probe; rowset spellings are right): `select item_desc as d, quantity as q, count(order_number) by item_desc as n` raises `No FINAL contributor renders ['local.q']` (main raised a different error); `select item_sk, item_desc, count(order_number), sum(quantity) by item_desc` gives the guest NULL for the sum where `sum by item_desc` groups it with the NULL-description items (the FINAL reaches the aggregate through the item dimension, which has no guest row; `[18]` in the probe).
3. **Rowset witness leftovers**: 5(c), an unsplit boundary is not stamped as the region holder (see tried). Probed with a boundary read at two grains, beside `count by *`, a window and a row-stream derivation: no wrong rows found, so it stays a plan-shape item without a rows test.
...
- `tests/engine/test_duckdb_rowset_null_group_rejoin.py`: `test_unsold_item_counts_no_lines` (direct, rowset, nested rowset), `RENAME_BESIDE_HASH_CASES`, `GUEST_CASES`, `GUEST_ALLDESC_CASES` (a `~?` guest with every item described: the NULL group has no member to pair with), the rowset-vs-direct pairs in `test_rowset_matches_direct_spelling` on both models, and the two-property models (`TWO_PROP_MODEL`, `TWO_PROP_GUEST_ALLDESC_MODEL`: the stand-in beside a second property, the stand-in as a join key, the unprojected-property WHERE).

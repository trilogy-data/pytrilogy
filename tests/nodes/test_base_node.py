...
property product_id.price float;
    ...
    x = StrategyNode(
        input_concepts=[], output_concepts=[env.concepts['order_id'], env.concepts['product_id']], environment=env, g=g ,
        partial_concepts = [env.concepts['order_id']]
    )


    y = StrategyNode(
        input_concepts=[], output_concepts=[env.concepts['product_id'], env.concepts['price']], environment=env, g=g,
        partial_concepts = []
    )

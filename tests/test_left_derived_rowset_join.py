"""A LEFT scoped join on a DERIVED-expression key between two re-aggregated
...
That works for INNER (the other rowset's key keeps its identity), but a LEFT
scoped join SUBSTITUTES the optional side's key onto the anchor's derived key —
...
LEFT must raise a clean error rather than a bare RecursionError.
"""

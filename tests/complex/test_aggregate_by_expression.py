...
def test_aggregate_by_paren_expression_parses() -> None:
    script = (
        _customer_env() + "select substring(phone, 1, 2) as cntrycode, "
...
def test_aggregate_by_paren_multi_expression() -> None:
    script = (
        _customer_env() + "select avg(acctbal) by (substring(phone, 1, 2), id) as a;"
    )
    env, parsed = parse(script)
    assert parsed[-1] is not None
    found = None
...
def test_aggregate_by_unparenthesized_expression_raises_helpful_error() -> None:
    import pytest

    from trilogy.core.exceptions import InvalidSyntaxException

    script = _customer_env() + "select avg(acctbal) by substring(phone, 1, 2) as a;"
    with pytest.raises(InvalidSyntaxException) as exc:
        parse(script)
    msg = str(exc.value)
    assert "211" in msg
    assert "wrapped in parens" in msg or "wrapped in parentheses" in msg.lower()

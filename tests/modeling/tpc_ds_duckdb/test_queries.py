...
def test_ninety_seven_alt(engine):
    ...
    assert len(query) < 5000, query
    # vacuous GROUP BY should be merged into its parent (yummy) and eliminated
    assert (
        "vacuous" not in query
    ), "Expected vacuous CTE to be merged away by MergeVacuousGroupBy"

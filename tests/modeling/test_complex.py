from preql.core.models import Environment,RowsetItem, AggregateWrapper
...
def test_rowset_with_aggregation(
    ...
    for count in [test_environment.concepts["local.even_order_count"], [x for x in statements[-1].output_components if x.name == "even_order_count"][0]]:

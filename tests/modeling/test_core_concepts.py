...
def test_property_assignments(test_environment: Environment):
    ...
    assert store_name.keys == {store_id.address}
    assert store_name.grain.components == {store_id.address}
    assert product_name.keys == {product_id.address}
...
def test_source_outputs(test_environment: Environment, test_executor: Executor):
    ...
    x = [
        ...
        for x in gen_select_node(
            [test_environment.concepts["store_id"], test_environment.concepts["order_id"]],

...
from preql.core.models import Datasource, Concept, ColumnAssignment, ConceptDeclaration, Environment, QueryDatasource
...
from preql.core.query_processor import get_query_datasources, source_query_concepts, generate_graph
from preql.core.processing.nodes import MergeNode, FilterNode, SelectNode
...
limit 5;'''
    ...
    filtered_node = gen_filter_node(
        env.concepts['surviving_passenger'],
          [family], env, g, 0, source_concepts
    )
    assert len(filtered_node.partial_concepts) == 2
    assert set([c.address for c in filtered_node.partial_concepts]) == set([c.address for c in [family, id]])
    ...
    assert set([c.address for c in resolved.partial_concepts]) == set([c.address for c in [family, id]])

    # check at the source level
    sourced = source_concepts( [family,env.concepts['surviving_passenger']], [], env, g, depth=0)
    ...
    assert set([c.address for c in mnode.partial_concepts]) == set([c.address for c in [family, id]])
    
    filter_node = sourced.parents[0]
    assert isinstance(filter_node, MergeNode)

    assert len(filter_node.partial_concepts) == 2
    assert set([c.address for c in filter_node.partial_concepts]) == set([c.address for c in [family, id]])


    # check at the query level
    filtered_source = source_query_concepts(output_concepts = [family,env.concepts['surviving_passenger']], environment=env)

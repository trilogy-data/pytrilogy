...
from preql.core.models import Concept, Datasource, ColumnAssignment, Function, ConceptTransform, Grain
...
def test_environment():
    env = Environment()
    order_id = Concept(name='order_id',
                       datatype = DataType.INTEGER,
                       purpose = Purpose.KEY)
    revenue = Concept(name = 'revenue',
    ...
    total_revenue = Concept(name = 'total_revenue',
                      datatype = DataType.FLOAT,
                      purpose = Purpose.METRIC,
                      lineage = Function(arguments=[revenue], output_datatype=DataType.FLOAT,
                                         output_purpose=Purpose.METRIC,
                                         operator = FunctionType.SUM))
    product_id = Concept(name = 'product_id',
                         datatype=DataType.INTEGER,
                         purpose = Purpose.KEY,
                         )

    category_id = Concept(name= 'category_id',
                          datatype = DataType.INTEGER,
                          purpose = Purpose.KEY)
    category_name = Concept(name = 'category_name',
    ...
    test_revenue = Datasource(
        identifier = 'revenue',
        columns= [ColumnAssignment(alias = 'revenue', concept=revenue),
    ...
    test_product = Datasource(
        identifier = 'products',
        columns = [ColumnAssignment(alias = 'product_id', concept = product_id),

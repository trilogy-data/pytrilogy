...
# regression (TPC-DS q74): rowset enrichment declined to source the other
# side of a SUBSET join (group-mate enrichment was coalescing-only), so the
# wb-side measures were sourced by a separate loop iteration that was never
# offered the already-attempted customer id; the stack fuse then silently
# re-correlated web totals on non-unique (first_name, last_name) — fanning
# each customer's totals out to every same-named customer. Rowset sourcing
# now follows the standard contract: subset members the author never
# references are pure correlation keys, so the enrichment sources the other
# side and merges over the declared relation locally; loop completion prunes
# a stack node that a sibling directly extends so the fuse cannot degrade
# the join surface.
MODEL = """
key csk int;

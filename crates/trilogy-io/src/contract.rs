...
impl Filter {
    ...
    pub fn parse(text: &str) -> Result<Filter> {
        ...
        let op = Op::parse(op_text).ok_or_else(|| {
            Error::contract(format!("Unsupported filter operator {op_text:?}"))
        })?;
        Ok(Filter {
...
/// `limit` rows and then has a filter applied to its output returns fewer rows
/// than asked for, so when there are filters to enforce locally the limit comes
/// back to the fallback -- correctness over the wasted scan.
pub fn effective_pushdown(declared: &[Field], request: &SourceRequest) -> Vec<Field> {
    let filters_local = request.is_set(Field::Filters) && !declared.contains(&Field::Filters);
    declared
        .iter()
        .copied()
        .filter(|field| !(filters_local && *field == Field::Limit))
        .collect()
}
...
mod tests {
    ...
    fn parses_operators_and_values() {
        assert_eq!(
            ...
            Filter { column: "i".into(), op: Op::GtEq, value: Value::from(5) }
        );
        assert_eq!(Filter::parse("state = CA").unwrap().value, Value::from("CA"));
        assert_eq!(Filter::parse(r#"state = "CA""#).unwrap().value, Value::from("CA"));
    ...
    fn limit_pushdown_is_withdrawn_when_filters_stay_local() {
        ...
        let plain = SourceRequest { limit: Some(4), ..Default::default() };
        assert_eq!(effective_pushdown(&[Field::Limit], &plain), vec![Field::Limit]);
    }

...
- `inner|left join <a> = <b>` is a QUERY-SCOPED join: the same model-blending as `merge`, but written inside one SELECT and applied only to that query. Place it after the (optional) WHERE and before SELECT. The LEFT key is the brought-in concept, the RIGHT key is the anchor: `where ... inner join attendance.student.id = enrollments.student.id select ...`. `inner join` asserts strict equivalence (like `merge a into b`); `left join` makes the brought-in side optional/nullable (like `merge a into ~b`). One clause per shared concept; stack multiple clauses (even onto the same model) for a multi-key blend. Chain `= c` to pull a base key into the join group so its properties stay reachable (`inner join a.k = b.k = base.k`). Only `inner` and `left` are supported. Prefer this over a separate `merge` for a query-local join; both are equivalent, never edit model files to wire a join. This is also the tool for SELF-PAIRING a model across two periods/sets (e.g. year-over-year): aggregate each period in its own `rowset` to shared keys, then `inner join` them on those keys. Full example: `trilogy agent-info syntax example scoped-join`.

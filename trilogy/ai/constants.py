...
- `inner|left|full join <a> = <b>` is a QUERY-SCOPED join, the DEFAULT, recommended way to blend two models inside one query (the same model-blending as `merge`, but local to this SELECT). Place it after the (optional) WHERE and before SELECT. Types match SQL; `inner join` asserts strict equivalence (drops unmatched rows); `left join` makes the right side optional/nullable (unmatched anchor rows kept); `full join` keeps unmatched rows from BOTH sides. `inner`, `left`, and `full` are all supported (`right` is not — swap the operands instead). A `full` key-group must be entirely full (it can't mix with `inner`/`left` on the same key; `full join a = b = c` chains one all-full group), while `inner` and `left` may mix freely. 
  * JOIN ON THE FULL GRAIN. When you blend two FACT models, write one `join` clause per key in their SHARED grain. `trilogy explore` prints each fact's grain as `@<k1, k2>` (e.g. `@<order_number, item.id>`); a composite-grain fact needs BOTH `inner join a.order_number = b.order_number` AND `inner join a.item.id = b.item.id`. Matching only ONE key of a multi-key grain fans out and double-counts — a top cause of wrong results. Stack a clause per shared ke.
  * Chain `= c` to pull addition aconcepts into a join (`inner join a.k = b.k = base.k`).
  * Full example (single-key blend, multi-key blend, and self-pair): `trilogy agent-info syntax example scoped-join`.
- merge <a> into <b> is the MODEL-LEVEL equivalent of a join (it persists for the whole query/file). PREFER a query-scoped `inner|left|full join` (above) for answering by default. Reach for `merge` only when the connection is universal to all queries. `merge a into ~b` marks `b` the superset so `a` is a partial subset (brought-in side nullable, like `left join`); plain `merge a into b` asserts strict equivalence (like `inner join`). One merge per shared concept.
- `union((armA), (armB), ...) -> (out1, out2, ...)` is the relational UNION table-valued function: it row-STACKS several self-contained `select` arms POSITIONALLY (SQL `UNION ALL`) into one named result. It can be used in a rowset `with combined as union(...) -> (...)` and reference outputs as `combined.out1`. Arms match by COLUMN POSITION (same count/order/types as the outputs). Arms can have full SQL including subfiltering and joins. Full example: `trilogy agent-info syntax example union-stack-channels`.
...
SELECT RULES:
- CLAUSE ORDER is fixed:-> `where` (per-row filter) -> `inner|left|full join`(s) -> `select` -> `having` -> `order by` -> `limit`. `where` ALWAYS precedes the join(s) and filters INPUT rows; using `having` to filter after the select.  For the full annotated example, fetch `trilogy agent-info syntax example query-structure`.
- No FROM, GROUP BY, DISTINCT, SELECT *, or SQL-style set operators. To STACK rows use the `union(...)` TVF; to BLEND fact models use a scoped `inner|left|full join a = b` — both documented above.
...
- A fact contains the full set of dimensional members, even those not in facts.
...
- All fields exist in a global namespace; field paths look like `enroll.student.id`. Always use the full path. 
- Newly created fields at the output of the select must be aliased with as (e.g. `sum(births) as all_births`).
- Aliases cannot happen inside calculations or in the where/having/order clause.  'sum(credits) as total_credits' is valid, but '(sum(credits) as total_credits) +1 as credits_plus_one' is not. Never alias fields to an existing name.
...
- Aggregates can be grouped inline instead of at query auto grain - ex:  `sum(metric) by dim1, dim2 as sum_by_dim1_dm2` for alternate grouping.
- The `by` clause accepts bare identifiers (`by dim1, dim2`) OR arbitrary expressions wrapped in parens (`by (substring(phone, 1, 2), upper(name))`). Use parens whenever a `by` entry is anything other than a simple identifier — function calls, casts, arithmetic, etc. — e.g. `avg(price) by (substring(phone, 1, 2))`. 
...
- Self-referential queries — relating a row to OTHER rows of the same set (period-over-period, previous/next value, running total, share of a group total, rank): Default to WINDOW functions. 
- To combine on an unnatural key (merge on properties, etc), use select joins.  STACK two subsets/channels as rows (not pair them on a key), use the `union(...)` TVF.
...
- Multi-level grouping (ROLLUP / CUBE / GROUPING SETS) attaches to an aggregate with a `by` clause and computes that aggregate at multiple grain levels in one pass:
  ...
  * The `by rollup|cube|grouping sets ...` clause attaches to ONE aggregate. When several aggregates need the same expansion, wrap them in a `def` macro so the rollup spec stays consistent or repeate it for each one:

...
`require_a_source_of_truth` (in `common.py`) fails a refresh that found nothing to do over an asset that is empty and has no root to compare against. With no `root datasource` every existing asset reads fresh, including a target table that holds no rows, so "up to date" plus exit 2 is indistinguishable from success.
...
A target whose table does not exist never reaches the guard: the state store reports it stale (`table not found`) on its own evidence, roots or not.

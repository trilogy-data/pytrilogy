        ...
        for current_concept in datasource.output_concepts:
            ...
            if current_derivation not in (Derivation.ROOT, Derivation.CONSTANT):
                ...
                if (
                    ...
                    new_concept = current_concept.model_copy(deep=True, update={"name": persisted})
                    self.add_concept(
                        new_concept, meta=meta, force=True, _ignore_cache=True
                    )
                    current_concept = current_concept.model_copy(deep=True, update={'lineage':None, 'metadata': current_concept.metadata.model_copy(update = {'concept_source':ConceptSource.PERSIST_STATEMENT})}).with_default_grain()
                    self.add_concept(

...
def get_graph_partial_nodes(
    g: nx.DiGraph, conditions: BuildWhereClause | None, cache: GraphAttributeCache = None
) -> dict[str, list[str]]:
...
def get_graph_grains(g: nx.DiGraph, cache: GraphAttributeCache = None) -> dict[str, list[str]]:
...
def create_pruned_concept_graph(
    ...
    for n in g_cache.ds_nodes:  # Only iterate over datasource nodes
        ...
        if any(neighbor in relevant_concepts_set for neighbor in nx.all_neighbors(g, n)):
...
def resolve_subgraphs(
    ...
    for key in subgraphs:
        ...
        for other_key in concept_map:
            ...
            if len(value) > len(other_value) or len(all_concepts) > len(other_all_concepts):
                continue
            
            other_value_set = set(other_value)
            other_all_concepts_set = set(other_all_concepts)
            
            if value_set.issubset(other_value_set) and all_concepts_set.issubset(other_all_concepts_set):
...
def create_select_node(
    ...
    all_concepts = [
        environment.concepts[extract_address(c)] 
        for c in subgraph 
        if c.startswith("c~")
    ]

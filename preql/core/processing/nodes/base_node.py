...
def resolve_concept_map(inputs: List[QueryDatasource], targets: List[Concept] = None):
    ...
    for target in targets:
        if target.lineage and not any(target in input.output_concepts for input in inputs):
...
class StrategyNode:
    ...
    def _resolve(self) -> QueryDatasource:
        ...
        source_map = resolve_concept_map(parent_sources,unique(self.all_concepts, "address") )

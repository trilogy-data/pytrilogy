...
def datasource_to_ctes(query_datasource:QueryDatasource)->List[CTE]:
    ...
    if len(query_datasource.datasources) > 1:
        ...
        for datasource in query_datasource.datasources:
            ...
            concepts = [c for c in query_datasource.concepts if c.name in sub_select.keys()]
        ...
        source_map = {concept.address:source.identifier for concept in query_datasource.concepts}
    output.append(CTE(
        name=f"cte_{int_id}",
        source=query_datasource,
        # output columns are what are selected/grouped by
        output_columns = query_datasource.concepts,
        source_map = source_map,
        # related columns include all referenced columns, such as filtering
        # related_columns=datasource.concepts,
        related_columns=query_datasource.concepts,

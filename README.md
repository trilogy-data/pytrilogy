...
# Plot it
trilogy run --import flight "chart layer barh ( y_axis <- carrier.name, x_axis <- count(id) as flight_count ) order by flight_count desc limit 10 layer barh (y_axis <-carrier.name, x_axis <-count(aircraft.id) as airplane_count) order by airplane_count desc limit 10;"

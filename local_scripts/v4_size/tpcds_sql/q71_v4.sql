
WITH 
cheerful as (
SELECT
    "sales_catalog_sales_unified"."CS_EXT_SALES_PRICE" as "sales_ext_sales_price",
    "sales_catalog_sales_unified"."CS_ITEM_SK" as "sales_item_sk",
    "sales_catalog_sales_unified"."CS_SOLD_TIME_SK" as "sales_sale_time_sk"
FROM
    "memory"."catalog_sales" as "sales_catalog_sales_unified"
    INNER JOIN "memory"."item" as "sales_item_items" on "sales_catalog_sales_unified"."CS_ITEM_SK" = "sales_item_items"."I_ITEM_SK"
    INNER JOIN "memory"."date_dim" as "sales_sale_date_date" on "sales_catalog_sales_unified"."CS_SOLD_DATE_SK" = "sales_sale_date_date"."D_DATE_SK"
    INNER JOIN "memory"."time_dim" as "sales_sale_time_time" on "sales_catalog_sales_unified"."CS_SOLD_TIME_SK" = "sales_sale_time_time"."T_TIME_SK"
WHERE
    "sales_item_items"."I_MANAGER_ID" = 1 and "sales_sale_date_date"."D_YEAR" = 1999 and "sales_sale_date_date"."D_MOY" = 11 and ( "sales_sale_time_time"."T_MEAL_TIME" = 'breakfast' or "sales_sale_time_time"."T_MEAL_TIME" = 'dinner' )

UNION ALL
SELECT
    "sales_store_sales_unified"."SS_EXT_SALES_PRICE" as "sales_ext_sales_price",
    "sales_store_sales_unified"."SS_ITEM_SK" as "sales_item_sk",
    "sales_store_sales_unified"."SS_SOLD_TIME_SK" as "sales_sale_time_sk"
FROM
    "memory"."store_sales" as "sales_store_sales_unified"
    INNER JOIN "memory"."item" as "sales_item_items" on "sales_store_sales_unified"."SS_ITEM_SK" = "sales_item_items"."I_ITEM_SK"
    INNER JOIN "memory"."date_dim" as "sales_sale_date_date" on "sales_store_sales_unified"."SS_SOLD_DATE_SK" = "sales_sale_date_date"."D_DATE_SK"
    INNER JOIN "memory"."time_dim" as "sales_sale_time_time" on "sales_store_sales_unified"."SS_SOLD_TIME_SK" = "sales_sale_time_time"."T_TIME_SK"
WHERE
    "sales_item_items"."I_MANAGER_ID" = 1 and "sales_sale_date_date"."D_YEAR" = 1999 and "sales_sale_date_date"."D_MOY" = 11 and ( "sales_sale_time_time"."T_MEAL_TIME" = 'breakfast' or "sales_sale_time_time"."T_MEAL_TIME" = 'dinner' )

UNION ALL
SELECT
    "sales_web_sales_unified"."WS_EXT_SALES_PRICE" as "sales_ext_sales_price",
    "sales_web_sales_unified"."WS_ITEM_SK" as "sales_item_sk",
    "sales_web_sales_unified"."WS_SOLD_TIME_SK" as "sales_sale_time_sk"
FROM
    "memory"."web_sales" as "sales_web_sales_unified"
    INNER JOIN "memory"."item" as "sales_item_items" on "sales_web_sales_unified"."WS_ITEM_SK" = "sales_item_items"."I_ITEM_SK"
    INNER JOIN "memory"."date_dim" as "sales_sale_date_date" on "sales_web_sales_unified"."WS_SOLD_DATE_SK" = "sales_sale_date_date"."D_DATE_SK"
    INNER JOIN "memory"."time_dim" as "sales_sale_time_time" on "sales_web_sales_unified"."WS_SOLD_TIME_SK" = "sales_sale_time_time"."T_TIME_SK"
WHERE
    "sales_item_items"."I_MANAGER_ID" = 1 and "sales_sale_date_date"."D_YEAR" = 1999 and "sales_sale_date_date"."D_MOY" = 11 and ( "sales_sale_time_time"."T_MEAL_TIME" = 'breakfast' or "sales_sale_time_time"."T_MEAL_TIME" = 'dinner' )
)
SELECT
    "sales_item_items"."I_BRAND_ID" as "brand_id",
    "sales_item_items"."I_BRAND" as "brand",
    "sales_sale_time_time"."T_HOUR" as "t_hour",
    "sales_sale_time_time"."T_MINUTE" as "t_minute",
    sum("cheerful"."sales_ext_sales_price") as "ext_price"
FROM
    "cheerful"
    INNER JOIN "memory"."item" as "sales_item_items" on "cheerful"."sales_item_sk" = "sales_item_items"."I_ITEM_SK"
    LEFT OUTER JOIN "memory"."time_dim" as "sales_sale_time_time" on "cheerful"."sales_sale_time_sk" = "sales_sale_time_time"."T_TIME_SK"
GROUP BY
    1,
    2,
    3,
    4
ORDER BY 
    "ext_price" desc nulls first,
    "brand_id" asc nulls first,
    "t_hour" asc nulls first,
    "t_minute" asc nulls first
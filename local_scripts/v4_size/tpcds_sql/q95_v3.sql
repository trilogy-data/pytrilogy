...
GROUP BY
    1
HAVING
    count("questionable"."web_sales_warehouse_sk") > 1
),
thoughtful as (
SELECT
...
WHERE
    cast("web_sales_ship_date_date"."D_DATE" as date) BETWEEN date '1999-02-01' AND date '1999-04-02' and "web_sales_pos_ship_address_customer_address"."CA_STATE" = 'IL' and "web_sales_web_site_web_site"."web_company_name" = 'pri' and exists (select 1 from abundant where abundant."multi_warehouse_order" is not distinct from "web_sales_web_sales"."WS_ORDER_NUMBER") and exists (select 1 from thoughtful where thoughtful."returned_orders" is not distinct from "web_sales_web_sales"."WS_ORDER_NUMBER")
),
cooperative as (
SELECT
    "web_sales_web_sales"."WS_ORDER_NUMBER" as "web_sales_order_number"
FROM
    ...
    INNER JOIN "memory"."customer_address" as "web_sales_pos_ship_address_customer_address" on "web_sales_web_sales"."WS_SHIP_ADDR_SK" = "web_sales_pos_ship_address_customer_address"."CA_ADDRESS_SK"
WHERE
    "web_sales_web_site_web_site"."web_company_name" = 'pri' and cast("web_sales_ship_date_date"."D_DATE" as date) BETWEEN date '1999-02-01' AND date '1999-04-02' and "web_sales_pos_ship_address_customer_address"."CA_STATE" = 'IL' and exists (select 1 from abundant where abundant."multi_warehouse_order" is not distinct from "web_sales_web_sales"."WS_ORDER_NUMBER") and exists (select 1 from thoughtful where thoughtful."returned_orders" is not distinct from "web_sales_web_sales"."WS_ORDER_NUMBER")


WITH 
sparkling as (
SELECT
    "cs_return_customer_current_address_customer_address"."CA_CITY" as "cs_return_customer_current_address_city",
    "cs_return_customer_current_address_customer_address"."CA_COUNTRY" as "cs_return_customer_current_address_country",
    "cs_return_customer_current_address_customer_address"."CA_COUNTY" as "cs_return_customer_current_address_county",
    "cs_return_customer_current_address_customer_address"."CA_GMT_OFFSET" as "cs_return_customer_current_address_gmt_offset",
    "cs_return_customer_current_address_customer_address"."CA_LOCATION_TYPE" as "cs_return_customer_current_address_location_type",
    "cs_return_customer_current_address_customer_address"."CA_STATE" as "cs_return_customer_current_address_state",
    "cs_return_customer_current_address_customer_address"."CA_STREET_NAME" as "cs_return_customer_current_address_street_name",
    "cs_return_customer_current_address_customer_address"."CA_STREET_NUMBER" as "cs_return_customer_current_address_street_number",
    "cs_return_customer_current_address_customer_address"."CA_STREET_TYPE" as "cs_return_customer_current_address_street_type",
    "cs_return_customer_current_address_customer_address"."CA_SUITE_NUMBER" as "cs_return_customer_current_address_suite_number",
    "cs_return_customer_current_address_customer_address"."CA_ZIP" as "cs_return_customer_current_address_zip",
    "cs_return_customer_customers"."C_CUSTOMER_ID" as "cs_return_customer_id",
    "cs_return_customer_customers"."C_CUSTOMER_SK" as "cs_return_customer_sk",
    "cs_return_customer_customers"."C_FIRST_NAME" as "cs_return_customer_first_name",
    "cs_return_customer_customers"."C_LAST_NAME" as "cs_return_customer_last_name",
    "cs_return_customer_customers"."C_SALUTATION" as "cs_return_customer_salutation"
FROM
    "memory"."customer" as "cs_return_customer_customers"
    INNER JOIN "memory"."customer_address" as "cs_return_customer_current_address_customer_address" on "cs_return_customer_customers"."C_CURRENT_ADDR_SK" = "cs_return_customer_current_address_customer_address"."CA_ADDRESS_SK"
WHERE
    "cs_return_customer_current_address_customer_address"."CA_STATE" = 'GA'
),
thoughtful as (
SELECT
    "cs_catalog_returns"."CR_RETURNING_CUSTOMER_SK" as "cs_return_customer_sk",
    "cs_return_address_customer_address"."CA_STATE" as "cs_return_address_state",
    sum(CASE WHEN "cs_return_date_date"."D_YEAR" = 2000 and "cs_return_address_customer_address"."CA_STATE" is not null THEN "cs_catalog_returns"."CR_RETURN_AMT_INC_TAX" ELSE NULL END) as "_virt_agg_sum_2893965424206230_wscope",
    sum(CASE WHEN "cs_return_date_date"."D_YEAR" = 2000 and "cs_return_address_customer_address"."CA_STATE" is not null THEN "cs_catalog_returns"."CR_RETURN_AMT_INC_TAX" ELSE NULL END) as "customer_state"
FROM
    "memory"."catalog_sales" as "cs_catalog_sales"
    LEFT OUTER JOIN "memory"."catalog_returns" as "cs_catalog_returns" on "cs_catalog_sales"."CS_ITEM_SK" = "cs_catalog_returns"."CR_ITEM_SK" AND "cs_catalog_sales"."CS_ORDER_NUMBER" = "cs_catalog_returns"."CR_ORDER_NUMBER"
    FULL JOIN "memory"."date_dim" as "cs_return_date_date" on "cs_catalog_returns"."CR_RETURNED_DATE_SK" = "cs_return_date_date"."D_DATE_SK"
    RIGHT OUTER JOIN "memory"."customer_address" as "cs_return_address_customer_address" on "cs_catalog_returns"."CR_RETURNING_ADDR_SK" = "cs_return_address_customer_address"."CA_ADDRESS_SK"
WHERE
    "cs_return_address_customer_address"."CA_STATE" is not null

GROUP BY
    1,
    2),
abundant as (
SELECT
    "thoughtful"."cs_return_address_state" as "cs_return_address_state",
    1.2 * avg("thoughtful"."_virt_agg_sum_2893965424206230_wscope") as "_virt_func_multiply_1237551001127604_wscope"
FROM
    "thoughtful"
GROUP BY
    1),
yummy as (
SELECT
    "thoughtful"."cs_return_customer_sk" as "cs_return_customer_sk",
    "thoughtful"."customer_state" as "customer_state"
FROM
    "thoughtful"
    INNER JOIN "abundant" on "thoughtful"."cs_return_address_state" is not distinct from "abundant"."cs_return_address_state"
WHERE
    "thoughtful"."customer_state" > "abundant"."_virt_func_multiply_1237551001127604_wscope"
)
SELECT
    "sparkling"."cs_return_customer_id" as "cs_return_customer_id",
    "sparkling"."cs_return_customer_salutation" as "cs_return_customer_salutation",
    "sparkling"."cs_return_customer_first_name" as "cs_return_customer_first_name",
    "sparkling"."cs_return_customer_last_name" as "cs_return_customer_last_name",
    "sparkling"."cs_return_customer_current_address_street_number" as "cs_return_customer_current_address_street_number",
    "sparkling"."cs_return_customer_current_address_street_name" as "cs_return_customer_current_address_street_name",
    "sparkling"."cs_return_customer_current_address_street_type" as "cs_return_customer_current_address_street_type",
    "sparkling"."cs_return_customer_current_address_suite_number" as "cs_return_customer_current_address_suite_number",
    "sparkling"."cs_return_customer_current_address_city" as "cs_return_customer_current_address_city",
    "sparkling"."cs_return_customer_current_address_county" as "cs_return_customer_current_address_county",
    "sparkling"."cs_return_customer_current_address_state" as "cs_return_customer_current_address_state",
    "sparkling"."cs_return_customer_current_address_zip" as "cs_return_customer_current_address_zip",
    "sparkling"."cs_return_customer_current_address_country" as "cs_return_customer_current_address_country",
    "sparkling"."cs_return_customer_current_address_gmt_offset" as "cs_return_customer_current_address_gmt_offset",
    "sparkling"."cs_return_customer_current_address_location_type" as "cs_return_customer_current_address_location_type",
    "yummy"."customer_state" as "customer_state"
FROM
    "yummy"
    INNER JOIN "sparkling" on "yummy"."cs_return_customer_sk" = "sparkling"."cs_return_customer_sk"
WHERE
    "sparkling"."cs_return_customer_current_address_state" = 'GA'

ORDER BY 
    "sparkling"."cs_return_customer_id" asc nulls first,
    "sparkling"."cs_return_customer_salutation" asc nulls first,
    "sparkling"."cs_return_customer_first_name" asc nulls first,
    "sparkling"."cs_return_customer_last_name" asc nulls first,
    "sparkling"."cs_return_customer_current_address_street_number" asc nulls first,
    "sparkling"."cs_return_customer_current_address_street_name" asc nulls first,
    "sparkling"."cs_return_customer_current_address_street_type" asc nulls first,
    "sparkling"."cs_return_customer_current_address_suite_number" asc nulls first,
    "sparkling"."cs_return_customer_current_address_city" asc nulls first,
    "sparkling"."cs_return_customer_current_address_county" asc nulls first,
    "sparkling"."cs_return_customer_current_address_state" asc nulls first,
    "sparkling"."cs_return_customer_current_address_zip" asc nulls first,
    "sparkling"."cs_return_customer_current_address_country" asc nulls first,
    "sparkling"."cs_return_customer_current_address_gmt_offset" asc nulls first,
    "sparkling"."cs_return_customer_current_address_location_type" asc nulls first,
    "yummy"."customer_state" asc nulls first
LIMIT (100)
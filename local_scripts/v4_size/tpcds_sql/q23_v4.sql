...
protective as (
SELECT
    "charming"."_best_customers_best_customer_id" as "best_customers_best_customer_id"
FROM
    "charming"),
yummy as (
SELECT
...
WHERE
    exists (select 1 from young where young."frequent_items_frequent_item_id" is not distinct from "sales_web_sales_unified"."WS_ITEM_SK") and exists (select 1 from protective where protective."best_customers_best_customer_id" is not distinct from "sales_web_sales_unified"."WS_BILL_CUSTOMER_SK") and "sales_sale_date_date"."D_YEAR" = 2000 and "sales_sale_date_date"."D_MOY" = 2
),
questionable as (
SELECT

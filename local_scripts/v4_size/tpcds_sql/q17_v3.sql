...
SELECT
    ...
    "vacuous"."analysis_item_id" as "analysis_item_id",
...
wakeful as (
SELECT
    "analysis_catalog_sales"."CS_BILL_CUSTOMER_SK" as "analysis_customer_sk",
    "analysis_catalog_sales"."CS_ITEM_SK" as "analysis_item_sk",
    "analysis_catalog_sales"."CS_SOLD_DATE_SK" as "analysis_catalog_date_sk"
FROM
...
FROM
    ...
    INNER JOIN "memory"."date_dim" as "analysis_store_sale_date_date" on "analysis_store_sales"."SS_SOLD_DATE_SK" = "analysis_store_sale_date_date"."D_DATE_SK"
    INNER JOIN "memory"."date_dim" as "analysis_store_return_date_date" on "analysis_store_returns"."SR_RETURNED_DATE_SK" = "analysis_store_return_date_date"."D_DATE_SK"
    INNER JOIN "wakeful" on "analysis_store_sales"."SS_CUSTOMER_SK" = "wakeful"."analysis_customer_sk" AND "analysis_store_sales"."SS_ITEM_SK" = "wakeful"."analysis_item_sk"
    INNER JOIN "memory"."date_dim" as "analysis_catalog_date_date" on "wakeful"."analysis_catalog_date_sk" = "analysis_catalog_date_date"."D_DATE_SK"
    LEFT OUTER JOIN "memory"."store" as "analysis_store_store" on "analysis_store_sales"."SS_STORE_SK" = "analysis_store_store"."S_STORE_SK"
    LEFT OUTER JOIN "memory"."item" as "analysis_item_items" on "analysis_store_sales"."SS_ITEM_SK" = "analysis_item_items"."I_ITEM_SK"
WHERE
    "analysis_store_sale_date_date"."D_YEAR" = 2001 and ("analysis_store_return_date_date"."D_YEAR" is not null and "analysis_store_return_date_date"."D_YEAR" in (2001,2002)) and ("analysis_catalog_date_date"."D_YEAR" is not null and "analysis_catalog_date_date"."D_YEAR" in (2001,2002)) and SR_RETURN_TIME_SK IS NOT NULL
...
GROUP BY
    ...
    4,
    5,
    coalesce("analysis_item_items"."I_ITEM_SK","analysis_store_returns"."SR_ITEM_SK","analysis_store_sales"."SS_ITEM_SK","wakeful"."analysis_item_sk"),
    coalesce("analysis_store_returns"."SR_TICKET_NUMBER","analysis_store_sales"."SS_TICKET_NUMBER")),
juicy as (
SELECT
...
FROM
    "concerned"
    INNER JOIN "juicy" on "concerned"."analysis_item_desc" is not distinct from "juicy"."analysis_item_desc" AND "concerned"."analysis_item_id" is not distinct from "juicy"."analysis_item_id" AND "concerned"."analysis_store_state" is not distinct from "juicy"."analysis_store_state"
ORDER BY 

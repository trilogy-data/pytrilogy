...
WHERE
    ("sales_item_items"."I_COLOR" is not null and "sales_item_items"."I_COLOR" in ('slate','blanched','burnished'))
...
WHERE
    exists (select 1 from cooperative where cooperative."color_ids" is not distinct from "sales_item_items"."I_ITEM_ID") and "sales_pos_bill_address_customer_address"."CA_GMT_OFFSET" = -5 and "sales_sale_date_date"."D_YEAR" = 2001 and "sales_sale_date_date"."D_MOY" = 2
...
SELECT
    ...
    sum("cheerful"."sales_ext_sales_price") as "total_sales"
FROM
    "cheerful"
GROUP BY
    1
ORDER BY 
    ...
    "cheerful"."sales_item_id" asc nulls first
LIMIT (100)
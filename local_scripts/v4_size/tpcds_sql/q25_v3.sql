
WITH 
vacuous as (
SELECT
    "analysis_catalog_sales"."CS_NET_PROFIT" as "analysis_catalog_net_profit",
    "analysis_item_items"."I_ITEM_DESC" as "analysis_item_desc",
    "analysis_item_items"."I_ITEM_ID" as "analysis_item_id",
    "analysis_store_store"."S_STORE_ID" as "analysis_store_id",
    "analysis_store_store"."S_STORE_NAME" as "analysis_store_name"
FROM
    "memory"."store_sales" as "analysis_store_sales"
    INNER JOIN "memory"."store_returns" as "analysis_store_returns" on "analysis_store_sales"."SS_CUSTOMER_SK" = "analysis_store_returns"."SR_CUSTOMER_SK" AND "analysis_store_sales"."SS_ITEM_SK" = "analysis_store_returns"."SR_ITEM_SK" AND "analysis_store_sales"."SS_TICKET_NUMBER" = "analysis_store_returns"."SR_TICKET_NUMBER"
    INNER JOIN "memory"."date_dim" as "analysis_store_sale_date_date" on "analysis_store_sales"."SS_SOLD_DATE_SK" = "analysis_store_sale_date_date"."D_DATE_SK"
    INNER JOIN "memory"."date_dim" as "analysis_store_return_date_date" on "analysis_store_returns"."SR_RETURNED_DATE_SK" = "analysis_store_return_date_date"."D_DATE_SK"
    INNER JOIN "memory"."catalog_sales" as "analysis_catalog_sales" on "analysis_store_sales"."SS_CUSTOMER_SK" = "analysis_catalog_sales"."CS_BILL_CUSTOMER_SK" AND "analysis_store_sales"."SS_ITEM_SK" = "analysis_catalog_sales"."CS_ITEM_SK"
    INNER JOIN "memory"."date_dim" as "analysis_catalog_date_date" on "analysis_catalog_sales"."CS_SOLD_DATE_SK" = "analysis_catalog_date_date"."D_DATE_SK"
    LEFT OUTER JOIN "memory"."store" as "analysis_store_store" on "analysis_store_sales"."SS_STORE_SK" = "analysis_store_store"."S_STORE_SK"
    LEFT OUTER JOIN "memory"."item" as "analysis_item_items" on "analysis_store_sales"."SS_ITEM_SK" = "analysis_item_items"."I_ITEM_SK"
WHERE
    "analysis_store_sale_date_date"."D_YEAR" = 2001 and "analysis_store_sale_date_date"."D_MOY" = 4 and "analysis_store_return_date_date"."D_YEAR" = 2001 and "analysis_store_return_date_date"."D_MOY" BETWEEN 4 AND 10 and "analysis_catalog_date_date"."D_YEAR" = 2001 and "analysis_catalog_date_date"."D_MOY" BETWEEN 4 AND 10 and SR_RETURN_TIME_SK IS NOT NULL

GROUP BY
    1,
    2,
    3,
    4,
    5,
    "analysis_catalog_sales"."CS_ORDER_NUMBER",
    coalesce("analysis_catalog_sales"."CS_ITEM_SK","analysis_item_items"."I_ITEM_SK","analysis_store_returns"."SR_ITEM_SK","analysis_store_sales"."SS_ITEM_SK")),
concerned as (
SELECT
    "vacuous"."analysis_item_desc" as "analysis_item_desc",
    "vacuous"."analysis_item_id" as "analysis_item_id",
    "vacuous"."analysis_store_id" as "analysis_store_id",
    "vacuous"."analysis_store_name" as "analysis_store_name",
    sum("vacuous"."analysis_catalog_net_profit") as "catalog_sales_profit"
FROM
    "vacuous"
GROUP BY
    1,
    2,
    3,
    4),
wakeful as (
SELECT
    "analysis_catalog_sales"."CS_BILL_CUSTOMER_SK" as "analysis_customer_sk",
    "analysis_catalog_sales"."CS_ITEM_SK" as "analysis_item_sk",
    "analysis_catalog_sales"."CS_SOLD_DATE_SK" as "analysis_catalog_date_sk"
FROM
    "memory"."catalog_sales" as "analysis_catalog_sales"
GROUP BY
    1,
    2,
    3),
yummy as (
SELECT
    "analysis_item_items"."I_ITEM_DESC" as "analysis_item_desc",
    "analysis_item_items"."I_ITEM_ID" as "analysis_item_id",
    "analysis_store_returns"."SR_NET_LOSS" as "analysis_store_return_net_loss",
    "analysis_store_sales"."SS_NET_PROFIT" as "analysis_store_net_profit",
    "analysis_store_store"."S_STORE_ID" as "analysis_store_id",
    "analysis_store_store"."S_STORE_NAME" as "analysis_store_name"
FROM
    "memory"."store_sales" as "analysis_store_sales"
    INNER JOIN "memory"."store_returns" as "analysis_store_returns" on "analysis_store_sales"."SS_CUSTOMER_SK" = "analysis_store_returns"."SR_CUSTOMER_SK" AND "analysis_store_sales"."SS_ITEM_SK" = "analysis_store_returns"."SR_ITEM_SK" AND "analysis_store_sales"."SS_TICKET_NUMBER" = "analysis_store_returns"."SR_TICKET_NUMBER"
    INNER JOIN "memory"."date_dim" as "analysis_store_sale_date_date" on "analysis_store_sales"."SS_SOLD_DATE_SK" = "analysis_store_sale_date_date"."D_DATE_SK"
    INNER JOIN "memory"."date_dim" as "analysis_store_return_date_date" on "analysis_store_returns"."SR_RETURNED_DATE_SK" = "analysis_store_return_date_date"."D_DATE_SK"
    INNER JOIN "wakeful" on "analysis_store_sales"."SS_CUSTOMER_SK" = "wakeful"."analysis_customer_sk" AND "analysis_store_sales"."SS_ITEM_SK" = "wakeful"."analysis_item_sk"
    INNER JOIN "memory"."date_dim" as "analysis_catalog_date_date" on "wakeful"."analysis_catalog_date_sk" = "analysis_catalog_date_date"."D_DATE_SK"
    LEFT OUTER JOIN "memory"."store" as "analysis_store_store" on "analysis_store_sales"."SS_STORE_SK" = "analysis_store_store"."S_STORE_SK"
    LEFT OUTER JOIN "memory"."item" as "analysis_item_items" on "analysis_store_sales"."SS_ITEM_SK" = "analysis_item_items"."I_ITEM_SK"
WHERE
    "analysis_store_sale_date_date"."D_YEAR" = 2001 and "analysis_store_sale_date_date"."D_MOY" = 4 and "analysis_store_return_date_date"."D_YEAR" = 2001 and "analysis_store_return_date_date"."D_MOY" BETWEEN 4 AND 10 and "analysis_catalog_date_date"."D_YEAR" = 2001 and "analysis_catalog_date_date"."D_MOY" BETWEEN 4 AND 10 and SR_RETURN_TIME_SK IS NOT NULL

GROUP BY
    1,
    2,
    3,
    4,
    5,
    6,
    coalesce("analysis_item_items"."I_ITEM_SK","analysis_store_returns"."SR_ITEM_SK","analysis_store_sales"."SS_ITEM_SK","wakeful"."analysis_item_sk"),
    coalesce("analysis_store_returns"."SR_TICKET_NUMBER","analysis_store_sales"."SS_TICKET_NUMBER")),
juicy as (
SELECT
    "yummy"."analysis_item_desc" as "analysis_item_desc",
    "yummy"."analysis_item_id" as "analysis_item_id",
    "yummy"."analysis_store_id" as "analysis_store_id",
    "yummy"."analysis_store_name" as "analysis_store_name",
    sum("yummy"."analysis_store_net_profit") as "store_sales_profit",
    sum("yummy"."analysis_store_return_net_loss") as "store_returns_loss"
FROM
    "yummy"
GROUP BY
    1,
    2,
    3,
    4)
SELECT
    "juicy"."analysis_item_id" as "analysis_item_id",
    "juicy"."analysis_item_desc" as "analysis_item_desc",
    "juicy"."analysis_store_id" as "analysis_store_id",
    "juicy"."analysis_store_name" as "analysis_store_name",
    "juicy"."store_sales_profit" as "store_sales_profit",
    "juicy"."store_returns_loss" as "store_returns_loss",
    "concerned"."catalog_sales_profit" as "catalog_sales_profit"
FROM
    "concerned"
    INNER JOIN "juicy" on "concerned"."analysis_item_desc" is not distinct from "juicy"."analysis_item_desc" AND "concerned"."analysis_item_id" is not distinct from "juicy"."analysis_item_id" AND "concerned"."analysis_store_id" is not distinct from "juicy"."analysis_store_id" AND "concerned"."analysis_store_name" is not distinct from "juicy"."analysis_store_name"
ORDER BY 
    "juicy"."analysis_item_id" asc,
    "juicy"."analysis_item_desc" asc,
    "juicy"."analysis_store_id" asc,
    "juicy"."analysis_store_name" asc
LIMIT (100)
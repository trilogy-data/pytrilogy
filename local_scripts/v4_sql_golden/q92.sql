...
SELECT
    ...
    1.3 * avg(CASE WHEN cast("ws_sale_date_date"."D_DATE" as date) BETWEEN :start_date AND :end_date THEN "ws_web_sales"."WS_EXT_DISCOUNT_AMT" ELSE NULL END) as "avg_item_disc"
FROM
    "memory"."web_sales" as "ws_web_sales"
    LEFT OUTER JOIN "memory"."date_dim" as "ws_sale_date_date" on "ws_web_sales"."WS_SOLD_DATE_SK" = "ws_sale_date_date"."D_DATE_SK"
WHERE
    "ws_web_sales"."WS_ITEM_SK" in (select "ws_item_sk" from "cheerful")
...
WHERE
    "cheerful"."ws_ext_discount_amount" > "thoughtful"."avg_item_disc"
